...
const priestleyTaylor = (qg: number, qo: number, rhmean: number, tmax: number, tmin: number, u2: number = null, elmsl: number) => {
...
export class PriestleyTaylorHandler extends EquationAbstractHandler {
    name: string = 'Priestley-Taylor';
    public handle(required: any): Eto {
        if (valid(['radQg', 'radQo', 'hum', 'tMax', 'tMin', 'windS', 'elevation'], required)) {
            ...
            const etoValue: number = priestleyTaylor(radQg, radQo, hum, tMax, tMin, windS, elevation);
            const eto: Eto = {
                equation: this.name,
                eto: etoValue,
            }
