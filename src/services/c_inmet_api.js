...
const a = (station, lat, lon, url, start_date, end_date, callback) => {
  ...
  request(url, async (err, response, html) => {
    ...
    config = {
      "aleaValue": "MDY0OA==",
      "xaleaValue": cod,
      "dtaini": dayjs(start_date, 'YYYYMMDD').subtract(1, 'day').format('DD/MM/YYYY'),
      "dtafim": dayjs(end_date, 'YYYYMMDD').format('DD/MM/YYYY'),
      "aleaNum": "0648",
      "xID": "272"
    }

    const mes = dayjs(start_date, 'YYYYMMDD').month() + 1;
    console.log("Mes para Rad", mes);
    console.log('Lat para rad', lat);

    let rad_q0 = Tools.getGlobalRadiation(mes, lat);
    ...
    request.post({ uri: url, formData: config }, (err, response, body) => {
      ...
      a.each(function (i, elem) {
        ...
        if (run) {
          ...
          df = df.appendPair([1, { Date: dayjs(end_date, 'YYYYMMDD').format('YYYYMMDD'), UTC: utc, Tmax: tmax, Tmin: tmin, Hum: hum, Wind: wind, Rad_Qg: rad_qg, Rad_Q0: rad_q0 }]);
